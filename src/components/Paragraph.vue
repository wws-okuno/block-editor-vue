<template>
  ...
  >
    ...
    <!-- 画像編集エリア -->
    <ImageEdit
      v-if="(getConfig('allowImages') || item.allowImages == true) && editingImage"
      v-model="item.images"
    ...
    <template v-else>
      <div class="BEV-item-wrap">
        ...
        <!--ctg 特定クラスの画像はテキストを表示しない-->
        <component v-if="item.className != 'box_img'"
          @focus="activateItem()"
...
export default {
  ...
  created() {
    ...
    // ctg presetsのallowImagesに対応 各セクションで「item.allowImages == true」追加
    if (!this.getConfig("allowImages") && !this.item.allowImages) {
      // 画像の添付禁止
  ...
  methods: {
    ...
    // フォーカスを当てる
    focus() {
      //ctg 特定クラスの画像はテキストを表示しないので、if追加
...
export class Item extends ItemBase {
  ...
  getItem(element) {
    ...
    const item = {
      ...
      images: (this.getConfig("allowImages") || this.allowImages == true) ? images : [],
  ...
  getHtml(item, base_ind) {
    ...
    //ctg 特定クラスの画像はテキストを必須にしない
    if (!content && item.className != "box_img") return null;
    ...
    let tag =
      ...
      `<${this.getConfig("tagName")} class="${this.getConfig(
        "tagClassName"
      )} align-${item.imageAlign}${cls_name}">${nl}`;
    if ((this.getConfig("allowImages") || item.allowImages == true) && item.images && item.images.length) {
      ...
      item.images.forEach((image) => {
        ...
        if (image.caption) {
          tag +=
            this._indent(3) +
            `<div class="caption">${image.caption}</div>${nl}`;
        }
  ...
  normalizeItem(item) {
    ...
    if (["left", "center", "right"].indexOf(item.imageAlign) === -1)
      item.imageAlign = this.getConfig("defaultImageAlign");
