<template>
  <div ref="ve" class="BEV-visual-text">
    <transition name="fade">
      <div @click.stop v-show="flgMenu" ref="toolbar" class="BEV-toolbar">
        <div :class="{'BEV-v-reverse': toolbarVReverse}" class="BEV-toolbar-inner">
          <div
            v-show="flgMenu==='toolbar'"
            :class="{'BEV-wrap': editorWidth < toolbarWidth}"
            class="BEV-item-wrap"
          >
            <div
              @click.prevent="flgMenu='link';focusUrlInput();"
              :class="{active: linkUrl}"
              v-if="enabledLink"
              class="BEV-item"
            >
              <IconBase width="15" height="15">
                <IconLink :active="linkUrl" />
              </IconBase>
              {{$t('common.link')}}
            </div>
            <div @click.prevent="unlink" v-if="enabledLink" v-show="linkUrl" class="BEV-item">
              <IconBase width="15" height="15">
                <IconUnlink />
              </IconBase>
              {{$t('VisualText.unlink')}}
            ...
            <div
              ...
              class="BEV-item"
            >
              <IconBase v-if="type==='bold'" width="15" height="15">
                <IconBold :active="appliedStyles.indexOf(type)!==-1" />
              </IconBase>
              {{label}}
            </div>
            ...
            <div v-show="flgMenu==='link'" class="link-wrap">
              ...
              <div class="BEV-row BEV-switch">
                ...
                <button
                  ...
                  @click.stop="showFileBrowser($event)"
                  type="button"
                >{{$t('ImageEdit.openFileBrowser')}}</button>
              </div>
              <div class="BEV-row">
                <button
                  @click="linkUrl='';closeToolbar()"
                  type="button"
                  class="BEV-cancel"
  ...
  methods: {
    ...
    initQuill() {
      ...
      this.cssClasses.forEach((cls) => {
        let label;
        if (cls === "bold") {
          label = this.$t("VisualText.bold");
        } else if (cls === "link") {
          label = this.$t("VisualText.link");
            ...
            d.ops.some((arr) => {
              if (arr.attributes) {
                if (arr.attributes.link) {
                  ...
                  link_blank =
                    arr.attributes.link.target === "_blank" ? true : false;
    ...
    arrangeHtml(html) {
      ...
      _html =
        "<p>" + _html.replace(/<br(\s+[^>]+)?(\s*\/)?>/g, "</p><p>") + "</p>";
      return _html;
    },
    // 一部ブラウザで
    ...
    _onKeyEevent(ev) {
      if (
        (ev.type === "keydown" && ev.code === "Escape") ||
        ev.type === "blur" ||
        ev.type === "mousedown"
      ) {
        // エスケープが押されたとき、
    ...
    showFileBrowser (image) {
      ...
      this.showFileBrowserWindow(this.fileBrowserCallback,'link')
    },
    // ファイルブラウザからのコールバック
