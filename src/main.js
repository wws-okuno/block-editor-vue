    ...
    if (element) {
      ...
      const defaultOptions = {
        ...
        itemOrder: [                                      // メニュー上のアイテムの表示順序
          'Paragraph', 'Heading', 'List', 'Table', 'Column', 'Html','Block'
        ],
        ...
        rootButtons: true,                          // アンドゥ、リドゥ、HTMLコピーボタンの有効化
        ...
        Block: {
          tagName: 'div',                               // アイテムを識別するタグ名
          tagClassName: 'block-wrap',                   // アイテムを識別するクラス名
          dispName: null,                               // ラベル（指定がなければi18nで定義された表示名が使用される）
          jsonUrl:null,                                 // 一覧選択用のURL jsonで返すこと
          jsonList:[],                                // jsonセット jsonUrlを入れるとセットされる
          presets: null,                         // プリセットリスト
        },
      }
      // 設定値のコピー
...
//       if ( options.Block.jsonUrl ) {
//         axios.get(options.Block.jsonUrl).then(response => (options.Block.jsonList = response.data))
//       }
